...
import { VaultEntry } from '../../types/hierarchy';
...
const HierarchyNode: FC<{ entry: VaultEntry; depth: number }> = ({ entry, depth }) => {
  ...
  return (
    <li className="py-0.5">
      <div 
        className="flex items-center gap-0.5 text-xs rounded hover:bg-gray-100 dark:hover:bg-gray-700 py-0.5 px-1 leading-snug"
        style={{ paddingLeft }}
      >
      ...
      {entry.kind === 'directory' && entry.children && entry.children.length > 0 && (
        <ul className="list-none">
          {entry.children.map((child) => (
            <HierarchyNode key={child.path} entry={child} depth={depth + 1} />
          ))}
        </ul>
      )}
    </li>
...
const HierarchyView: FC<HierarchyViewProps> = ({ hierarchyData, isLoading }) => {
  if (isLoading) {
    return (
    ...
    <div className="overflow-auto pl-1">
      ...
      <ul className="list-none ml-0.5">
        {hierarchyData.map((entry) => (
          <HierarchyNode key={entry.path} entry={entry} depth={0} />
        ))}
      </ul>
    </div>
  );
